...
class GymCoursePlayNotifier extends _$GymCoursePlayNotifier {
  ...
  /// 将目标值按 OpCode 对应的 FTMS 单位换算后，转为 2 字节小端序列（LE）。
  ///
  /// 换算规则（与 QuickStartNotifier 一致）：
  /// - 0x02 速度：值 × 100（FTMS 单位 0.01 km/h）
  /// - 0x03 坡度：值 × 10（FTMS 单位 0.1%）
  /// - 0x04 阻力：值原样（level）
  List<int> _buildValueBytes(int opCode, double value) {
    ...
    final clamped = raw & 0xFFFF; // sint16 负数自动转二补码
