...
/// 是否显示开始按钮（对应旧 hc.showPlayButton）。
 ...
 double get currentEnergyGoalKcal;/// 阻力指令下发后是否正在等待设备回执实际阻力值。
/// true 时中心按钮显示「获取中」，收到 0x07 回执或超时后置 false。
 bool get isFetchingResistance;/// 进入界面时检测到设备是否正在运行（speed/cadence/strokeRate > 0）。
/// true 时弹出引导用户先停止设备的阻塞弹窗，设备停转后自动置 false。
 bool get isDeviceRunningDetected;
...
as double,isFetchingResistance: null == isFetchingResistance ? _self.isFetchingResistance : isFetchingResistance // ignore: cast_nullable_to_non_nullable
...
/// true 时中心按钮显示「获取中」，收到 0x07 回执或超时后置 false。
@override@JsonKey() final  bool isFetchingResistance;
